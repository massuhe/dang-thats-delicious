...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(updateView.bind(this)).catch(console.error);
...
function putPlacesInMap(map, places) {
  ...
  var markers = places.map(function (place) {
    return makeMarker(map, place, bounds, infoWindow);
  });
...
function makeMarker(map, place, bounds, infoWindow) {
  ...
  marker.addListener('click', function () {
    var html = getMarkerInfoHTML(this);
...
function loadPLaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    return putPlacesInMap(map, res.data);
  });
}
...
function showResults(searchResults, stores) {
  ...
  var html = searchResultsHTML(stores);
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      return showResults(searchResults, res.data);
    }).catch(function (err) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    var currentActiveResult = results.findIndex(function (r) {
      return r.classList.contains('search__result--active');
    });
